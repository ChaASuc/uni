...
//        //拼接缓存键名（字符串）
//        String cacheName = CacheNameHelper.Receive_CacheNamePrefix +"id"+id;
//        //尝试在缓存中通过键名获取相应的键值
...
//            response.setContentType("application/json;charset=utf-8");
//            //拼接缓存键名（字符串）
//            String cacheName = CacheNameHelper.Receive_CacheNamePrefix +"pictureUserId"+ user_id;
//            //尝试在缓存中通过键名获取相应的键值
//            //因为在Redis中，数据是以”“” "键-值"对 的形式储存的
//            String json = cache.get(cacheName);
//            //如果在缓存中找不到，那就从数据库里找
...
//            if(success==1){
//                //清除相应的缓存
//                cache.delete(CacheNameHelper.Receive_CacheNamePrefix +"midifyPwd"+ user_id);
//                cache.delete(CacheNameHelper.ListAll_CacheName);
//                return Result.build(ResultType.Success);
//            }else{
//                return Result.build(ResultType.Failed);
...
//            response.setContentType("application/json;charset=utf-8");
//            //拼接缓存键名（字符串）
//            String cacheName = CacheNameHelper.Receive_CacheNamePrefix +"userInfoListAll"+ user_id;
//            //尝试在缓存中通过键名获取相应的键值
//            //因为在Redis中，数据是以”“” "键-值"对 的形式储存的
//            String json = cache.get(cacheName);
//            //如果在缓存中找不到，那就从数据库里找
