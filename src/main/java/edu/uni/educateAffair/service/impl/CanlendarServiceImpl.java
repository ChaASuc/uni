...
/*        for (Canlendar canlendar : c){
    ...
    @Override
    public Canlendar selectByCanlendarId(Long canlendarId) {
        CanlendarExample canlendarExample = new CanlendarExample();
        canlendarExample.createCriteria()
                .andIdEqualTo(canlendarId)
                .andDeletedEqualTo(isUse);
        List<Canlendar> canlendars = canlendarMapper.selectByExample(canlendarExample);
        if (canlendars.size() > 1) {
            try {
                throw new SQLException("根据学历id获取学历错误");
            } catch (SQLException e) {
                e.printStackTrace();
            }
        } else if (canlendars.size() == 0) {
            return null;
        }
        return canlendars.get(0);
    }
