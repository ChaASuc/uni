...
public class SpecialtyController {
    ...
    @GetMapping(value = "/Specialty/listLikeName/{name}")
    public void listLikeName(@PathVariable String name,
                                   HttpServletResponse response) throws IOException {
        response.setContentType("application/json;charset=utf-8");
//        String cacheName = CacheNameHelper.ListByDepartmentID_CacheNamePrefix + department_id+ "_" + pageNum;
        String json;
//        if(json == null){
            List<Specialty> specialties = specialtyService.selectSpecialtyLikeName(name);
            json = Result.build(ResultType.Success).appendData("specialties", specialties).convertIntoJSON();
//            if(pageInfo != null) {
//                cache.set(cacheName, json);
