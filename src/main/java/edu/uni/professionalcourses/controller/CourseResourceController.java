...
public class CourseResourceController {
    ...
    @GetMapping(value = "/courseResources/listByCourseId/{course_id}")
    public void listByCourseId(@PathVariable long course_id, HttpServletResponse response)throws IOException{
        response.setContentType("application/json;charset=utf-8");
        String json;
        List<CourseHasResource> courseHasResources=courseHasResourceService.selectByCourseId(course_id);
        List<CourseResource> courseResources=new ArrayList<>();
        for(int i=0;i<courseHasResources.size();i++){
            CourseResource courseResource=courseResourceService.select( courseHasResources.get(i).getCourseResourceId());
            courseResources.add(courseResource);
        }
        json=Result.build(ResultType.Success).appendData("courseResources",courseResources).convertIntoJSON();
        response.getWriter().write(json);
    }
}