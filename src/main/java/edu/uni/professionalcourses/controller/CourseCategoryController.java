...
public class CourseCategoryController {
    ...
    @GetMapping(value = "/courseCategorys/listAll")
    public void listAll(HttpServletResponse response) throws IOException {
        response.setContentType("application/json;charset=utf-8");
//        String cacheName = CacheNameHelper.ListByUniversityID_CacheNamePrefix + university_id + "_" + pageNum;
        String json;
//        if(json == null){
            List<CourseCategory> courseCategories = courseCategoryService.ListAll();
            json = Result.build(ResultType.Success).appendData("courseCategories", courseCategories).convertIntoJSON();
