...
//            @ApiImplicitParam(name="pageNum", value = "页码", required = true, dataType = "Integer", paramType = "path")
    ...
    @GetMapping(value = "/courseExperimentDescriptions/listByCourseexperimentId/{courseexperiment_id}")
    public void listByCourseexperimentID(@PathVariable long courseexperiment_id,
//                               @PathVariable Integer pageNum,
                               HttpServletResponse response) throws IOException {
        response.setContentType("application/json;charset=utf-8");
        String cacheName = CacheNameHelper.ListByCourseexperimentID_CacheNamePrefix + courseexperiment_id ;
        String json = cache.get(cacheName);
        if(json == null){
            List<CourseExperimentDescription> courseExperimentDescriptions = courseExperimentDescriptionService.selectPageByCourseexperiment(courseexperiment_id);
            json = Result.build(ResultType.Success).appendData("courseExperimentDescriptions", courseExperimentDescriptions).convertIntoJSON();
