...
//            @ApiImplicitParam(name="pageNum", value = "页码", required = true, dataType = "Integer", paramType = "path")
    ...
    @GetMapping(value = "/courseSyllabusDescriptions/listByCoursesyllabusId/{coursesyllabus_id}")
    public void listByCourseexperimentID(@PathVariable long coursesyllabus_id,
//                                         @PathVariable Integer pageNum,
                                         HttpServletResponse response) throws IOException {
        response.setContentType("application/json;charset=utf-8");
        String cacheName = CacheNameHelper.ListByCoursesyllabusID_CacheNamePrefix + coursesyllabus_id ;
        String json = cache.get(cacheName);
        if(json == null){
            List<CourseSyllabusDescription> courseSyllabusDescriptions = courseSyllabusDescriptionService.selectPageByCoursesyllabus( coursesyllabus_id);
            json = Result.build(ResultType.Success).appendData("courseSyllabusDescriptions", courseSyllabusDescriptions).convertIntoJSON();
//            if(pageInfo != null) {
//                cache.set(cacheName, json);
//            }
        }
