...
//import edu.uni.administrativestructure.controller.UniversityController;
...
public class CourseController {
    ...
    @GetMapping(value = "/courses/listAll")
    public void list(HttpServletResponse response) throws IOException {
        response.setContentType("application/json;charset=utf-8");
        String cacheName = CacheNameHelper.ListAll_CacheName;
        String json = cache.get(cacheName);
        if(json == null){
            json = Result.build(ResultType.Success).appendData("courses", courseService.selectAll()).convertIntoJSON();
            cache.set(cacheName, json);
        }
        response.getWriter().write(json);
    }
    @GetMapping(value = "/courses/listBySpecialtyId/{specialtyId}")
    public void listByCourseId(@PathVariable long specialtyId, HttpServletResponse response)throws IOException{
        response.setContentType("application/json;charset=utf-8");
        String json;
        List<Course> courses=courseService.selectBySpecialty(specialtyId);
//        List<Course> courses=new ArrayList<>();
//        for(int i=0;i<specialtyCourses.size();i++){
//            Course course=courseService.select( specialtyCourses.get(i).getCourseId());
//            courses.add(course);
//        }
    ...
    @PostMapping("Selectivecourse")
    @ResponseBody
    public void select(@RequestBody(required = false) Course course,HttpServletResponse response) throws IOException {
            response.setContentType("application/json;charset=utf-8");
            List<Course> courses = courseService.selective(course);
            String json;
            json=Result.build(ResultType.Success).appendData("courses",courses).convertIntoJSON();
            response.getWriter().write(json);
    }
