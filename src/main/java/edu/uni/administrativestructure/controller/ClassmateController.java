...
//            @ApiImplicitParam(name="pageNum", value = "页码", required = true, dataType = "Integer", paramType = "path")
    ...
    @GetMapping(value = "/classmates/listByClassID/{class_id}")
    public void listByClassID(@PathVariable long class_id,
//                                   @PathVariable Integer pageNum,
                                   HttpServletResponse response) throws IOException {
        response.setContentType("application/json;charset=utf-8");
        String cacheName = CacheNameHelper.ListByClassID_CacheNamePrefix + class_id;
        String json = cache.get(cacheName);
        if(json == null){
            List<Classmate> classmates = classmateService.selectPageByClass(class_id);
            json = Result.build(ResultType.Success).appendData("classmates", classmates).convertIntoJSON();
            if(classmates != null) {
                cache.set(cacheName, json);
            }
        }
