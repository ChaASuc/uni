...
public class DepartmentController {
    ...
    static class CacheNameHelper{
        ...
        public static final String ListLikeDepartmentName_name="as_department_selectLikeDepartmentName_";
    ...
    public Result create(@RequestBody(required = false) Department department){
        if(department != null){
            ...
            if(success){
                ...
                cache.deleteByPaterm(CacheNameHelper.ListLikeDepartmentName_name + "*");
     ...
     * @throws IOException
     */
    @ApiOperation(value="获取学院", notes="已测试")
    @ApiImplicitParam(name = "name", value = "部门名称", required = false, dataType = "String", paramType = "path")
    @GetMapping("/department/listLikeDepartmentName/{name}")
    public void receiveDepartment(@PathVariable String name, HttpServletResponse response) throws IOException {
        response.setContentType("application/json;charset=utf-8");
        String cacheName = CacheNameHelper.ListLikeDepartmentName_name+ name;
        String json = cache.get(cacheName);
        if(json == null){
            json = Result.build(ResultType.Success).appendData("department",departmentService.selectLikeDepartmentName(name)).convertIntoJSON();
            cache.set(cacheName, json);
        }
        response.getWriter().write(json);
    }
